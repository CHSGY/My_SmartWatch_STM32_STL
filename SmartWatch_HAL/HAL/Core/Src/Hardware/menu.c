/**
  * @file           : menu.c
  * @brief          : 主菜单及功能模块（HAL库版本）
  * @author         : CHSGY
  * @date           : 2026-06-07
  *
  * @note           : 从标准库menu.c移植，主要修改点：
  *                   1. 头文件：stm32f10x.h → main.h, Delay.h → delay.h
  *                   2. 延时函数：Delay_ms() → delay_ms()
  *                   3. 电源控制：GPIO直接操作 → power模块(Power_IsRunning/Power_Shutdown)
  *                   4. ADC初始化：AD_Init() 不再需要（CubeMX已完成配置）
  *                   5. 暂未迁移的模块(SetTime/dino)以stub形式提供，待后续迁移后替换
  */
...
uint8_t First_Page_Clock(void)
{
	while(1) 					//Get key's number continuely
	{
		...
		else if(KeyNum == 4) 	//Long Press Key3 → 电源控制
		{
			/**
			  * @note HAL版本：电源控制由power模块管理
			  *   原STD代码使用GPIO直接操作PB12/PB13
			  *   现改为判断运行状态后执行关机
...
uint8_t SettingPage(void)
{
	...
	while(1) //Get key's number continuely
	{
		...
		{
			Temp_SettingFlag = 0;
			SetTime_mainprocess();				//调用[设置日期时间]函数（stub，待迁移）
			OLED_Clear();
		}
...
int flashlight_Func(void)
{
	...
	while(1) //Get key's number continuely
	{
		...
		if(KeyNum == 1) //Key1: last item
		{
			...
			{
				flashlight_Flag = 3; 		//cursor move to second function： [ON]
			}
		}
		//when press key2 move to next item, cursor move to first item
		else if(KeyNum == 2) //Key2: next item
...
{
	delay_ms(MPU_SAMPLE_DELAY_MS);								//采样周期为5ms（原Delay_ms改为delay_ms）
...
int Game(void)
{
	...
	while(1) //Get key's number continuely
	{
		...
		{
			Game_Init();						//调用游戏初始化（stub，待dino迁移后替换）
			Dino_game_Animation();				//调用游戏动画（stub，待dino迁移后替换）
			Temp_game_flag = 0;
		}
		else
		{
...
void Show_emoji_UI(void)
{
	...
	for(uint8_t i=0; i<=EMOJI_BLINK_FRAMES; i++)
	{
		...
		OLED_Update();
		delay_ms(EMOJI_BLINK_DELAY_MS);		//原Delay_ms改为delay_ms
	}
	...
	delay_ms(EMOJI_BLINK_GAP_MS);				//原Delay_ms改为delay_ms
}
