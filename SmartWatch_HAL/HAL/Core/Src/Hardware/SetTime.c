/**
  * @file           : SetTime.c
  * @brief          : 日期时间设置功能（HAL库版本）
  * @author         : CHSGY
  * @date           : 2026-06-08
  *
  * @note           : 从标准库SetTime.c移植，主要修改点：
  *                   1. 头文件 stm32f10x.h → main.h
  *                   2. 头文件路径 OLED.h/Key.h/menu.h → Hardware/前缀
  */
...
void ChangeRTC_Time(uint8_t i,uint8_t flag)
{
	...
	MyRTC_SetTime(); 				//把更新后的时间值从结构体重新刷新到RTC硬件电路中
}

static uint8_t KeyNum;             /* 暂存按键键值 */

/**
  * @brief 设置年份
  * @retval 0-返回
  * @note  Key1加年份 Key2减年份 Key3确认并退出
  */
int Set_Year(void)
{
	while(1)
...
/**
  * @brief 设置月份
  * @retval 0-返回
  * @note  Key1加月份 Key2减月份 Key3确认并退出
  */
int Set_Month(void)
{
	while(1)
...
/**
  * @brief 设置日期
  * @retval 0-返回
  * @note  Key1加日期 Key2减日期 Key3确认并退出
  */
int Set_Day(void)
{
	while(1)
...
/**
  * @brief 设置小时
  * @retval 0-返回
  * @note  Key1加小时 Key2减小时 Key3确认并退出
  */
int Set_Hour(void)
{
	while(1)
...
/**
  * @brief 设置分钟
  * @retval 0-返回
  * @note  Key1加分钟 Key2减分钟 Key3确认并退出
  */
int Set_Min(void)
{
	while(1)
...
/**
  * @brief 设置秒钟
  * @retval 0-返回
  * @note  Key1加秒钟 Key2减秒钟 Key3确认并退出
  */
int Set_Sec(void)
{
	while(1)
