...
### 4.2 📐 `dino_tick()` 与 `KeyTick()` 中断职责拆分 ✅ 已确定架构

**当前架构：** 所有 tick 任务（按键、游戏、秒表）统一在 `TIM2_IRQHandler` 中执行。
...
| 因素 | 分析 |
|------|------|
| **任务特性** | 所有 tick 函数都是轻量级计数器操作（累加+条件判断），单次执行 < 10μs |
| **CPU 资源** | STM32F103 @72MHz，1ms 中断占用 CPU < 1%，资源极其充裕 |
| **主循环瓶颈** | 主循环含 `OLED_Update()`（SPI/I2C 传输 1-10ms）等耗时操作，若将 tick 任务移至主循环会导致响应延迟 |
| **实时性要求** | 按键检测依赖精确的 20ms 消抖定时，秒表需要准确的 1s 计时，游戏物理需要稳定的帧率 |

**曾尝试过的方案（已验证不适合本项目）：**

| 方案 | 问题 |
|------|------|
| 中断仅做 `sys_tick++`，主循环调度所有任务 | 按键响应延迟严重，需要长按才能触发 |
| 中断执行按键任务，主循环执行其他任务 | 秒表和游戏响应变慢，实时性无法保证 |

**结论：** 对于本项目（裸机超级循环 + 轻量级 tick 任务 + 主循环有耗时操作），**所有 tick 任务在中断中执行是最优架构**。只有当项目演进到 RTOS 架构（任务独立调度、中断仅做标记）时，才需要考虑将业务逻辑移至任务中。

**未来扩展建议：**

如果需要增加新的 tick 任务（如传感器采样），只要确保任务足够轻量（< 50μs），仍可在中断中执行。若需执行较重的计算逻辑，应考虑：
1. 在中断中设置标志位，主循环中轮询执行
2. 或移植 FreeRTOS，将重逻辑移至独立任务
